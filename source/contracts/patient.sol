...
contract EHR {
    ...
    modifier onlyDoctor(uint _doctorId) {
        require(
            doctors[_doctorId].createdBy == msg.sender,
            "Only the registered doctor can perform this action"
        );
        _;
    }
    ...
    modifier onlyPatient(uint _patientId) {
        require(
            ...
            "Only the registered patient can perform this action"
        );
        _;
    }
    ...
    function viewDoctor(
        ...
        require(
            doctors[_doctorId].createdBy == msg.sender,
            "You are not authorized to view this doctor's data"
        );
    ...
    function viewPatientMedicines(
        uint _patientId
    ) public view returns (uint[] memory) {
        require(
            patients[_patientId].createdBy == msg.sender,
            "You are not authorized to view this patient's medicines"
        );
